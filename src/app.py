...
def play_next_sentence(queue: list):
    """
    Pop the next audio path from the queue and return it as the new audio value.
    Called automatically by JS when the current sentence finishes playing.
    """
    if not queue:
        return gr.update(value=None), []
    next_path = queue[0]
    remaining  = queue[1:]
    print(f"[Audio System] Advancing to next sentence. {len(remaining)} left in queue.")
    return gr.update(value=next_path, autoplay=True), remaining


def reset_audio(queue: list):
    """Stop audio and clear the queue (used by the Interrupt button)."""
    return gr.update(value=None), []
...
def process_voice_interaction(audio_filepath, chat_history):
    ...
    try:
        ...
        # Update chat immediately so user sees the text
        chat_history.append({"role": "user",      "content": user_text_query})
        chat_history.append({"role": "assistant",  "content": full_reply_text})
        yield chat_history, gr.update(value=None), []

        # 3. Pre-generate all sentence audio files
        sentences = [s.strip() for s in re.split(r'(?<=[.!?]) +', full_reply_text) if s.strip()]
        audio_paths = []
        for index, sentence in enumerate(sentences):
            print(f"[Audio System] Synthesising {index + 1}/{len(sentences)}: '{sentence[:50]}'")
            path = generate_tesla_audio(sentence, chunk_index=index)
            if path:
        ...
        # Play first sentence; rest go into the queue
        first  = audio_paths[0]
        remaining = audio_paths[1:]
        yield chat_history, gr.update(value=first, autoplay=True), remaining
...
def process_text_interaction(user_text_message, chat_history):
    ...
    # Show text immediately, clear old audio
    yield "", chat_history, gr.update(value=None), []

    # Pre-generate all sentence audio files
    sentences = [s.strip() for s in re.split(r'(?<=[.!?]) +', full_reply_text) if s.strip()]
    audio_paths = []
    for index, sentence in enumerate(sentences):
        print(f"[Audio System] Synthesising {index + 1}/{len(sentences)}: '{sentence[:50]}'")
...
with gr.Blocks(title="Tesla Digital Twin") as demo:
    ...
    # Holds the remaining audio file paths to play sequentially
    audio_queue = gr.State([])

    with gr.Row():
        with gr.Column(scale=3):
            chatbot = gr.Chatbot(height=500, label="Tesla")
    ...
    voice_event = audio_input.stop_recording(
        ...
        outputs=[chatbot, audio_output, audio_queue]
    )
    text_event = text_input.submit(
        fn=process_text_interaction,
        inputs=[text_input, chatbot],
        outputs=[text_input, chatbot, audio_output, audio_queue]
    )

    # KEY: when audio finishes playing naturally, Gradio fires .stop().
    # We pop the next sentence from the queue and load it immediately.
    # This self-chains: each new audio will also fire .stop() when done.
 
    audio_output.stop(
        fn=play_next_sentence,
        inputs=[audio_queue],
        outputs=[audio_output, audio_queue]
    )

    # Start recording cancels ongoing generation
    audio_input.start_recording(fn=lambda: None, cancels=[voice_event, text_event])

    # Interrupt: cancel generation and clear the queue
    stop_btn.click(fn=lambda: None, cancels=[voice_event, text_event])
    stop_btn.click(fn=reset_audio, inputs=[audio_queue], outputs=[audio_output, audio_queue])
